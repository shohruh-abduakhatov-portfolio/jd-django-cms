...
const ANGULAR_IP = 'http://178.218.200.63:80/';
const CMS_IP = 'http://178.218.200.63:8001/';
const DJANGO_IP = 'http://178.218.200.63:8000/';
...
function logOut() {
    localStorage.clear();
    console.log('logout' + getCookie('token'));
    token = params.delete(TOKEN_TITLE);
    sessionStorage.removeItem(TOKEN_TITLE);
    console.log(token);
    window.location.href = CMS_IP + "auth/logout?token="+getCookie('token');
...
function displayData() {
    ...
    if (localStorage['url'] && localStorage[lang]) {
        ...
        const ul = document.getElementById("userAccessPages");
        for (let i = 0; i < len; i++) {
            let li = window.document.createElement('li');
            let a = window.document.createElement('a');
            a.innerText = lang_stored[i];
            li.appendChild(a);
            ul.appendChild(li);
            if (i === 0) {
...
function getMenuItems() {
    let lang = window.location.pathname.slice(1, -1);
